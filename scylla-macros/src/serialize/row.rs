        ...
        let serialize_field_block: syn::Block = if self.ctx.fields.is_empty() {
            ...
            parse_quote! {{
                match spec.name() {
                    ...
                    _ => {
                        let mk_err = |err| {
                            #crate_path::SerializationError::new(#crate_path::BuiltinRowSerializationError {
                                rust_name: ::std::any::type_name::<#struct_name #ty_generics>(),
                                kind: #crate_path::BuiltinRowSerializationErrorKind::ColumnSerializationFailed {
                        ...
                        #({
                            let serialized =
                                self.#flattened_fields.serialize_field(spec, writer).map_err(mk_err)?;
                            match serialized {
